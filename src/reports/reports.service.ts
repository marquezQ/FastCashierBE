...
export class ReportsService {
  ...
  async getPaymentMethodsPerformance(
    ...
    const rawResults = await this.orderRepository
      ...
      .select(`DATE_TRUNC('${groupBy}', order.orderDate)`, 'date')
      .addSelect(
        `SUM(CASE WHEN order.paymentMethod = 'CASH' THEN order.total ELSE 0 END)`,
        'efectivo',
      )
    ...
    await doc.table(tableData, {
      ...
      prepareRow: (row, indexColumn, indexRow) => {
        ...
        if (indexRow === rows.length - 1) {
          return doc.font('Helvetica-Bold').fontSize(9);
        } else {
  ...
  async generateSingleSessionPdf(session: any, orders: Order[]): Promise<Buffer> {
    ...
    doc.text(
      `Apertura: ${formatDateTime(session.openingDate)}${session.closingDate ? '  |  Cierre: ' + formatDateTime(session.closingDate) : '  |  Cierre: PENDIENTE'}`,
    );
    ...
    await doc.table(tableData, {
      ...
      prepareRow: (row: any, indexColumn: number, indexRow: number) => {
        if (indexRow === rows.length - 1) {
          return doc.font('Helvetica-Bold').fontSize(9).fillColor('black');
        } else if (row[7] === 'Anulado') {
          return doc.font('Helvetica-Oblique').fontSize(8).fillColor('red');
        } else {
    ...
    doc
      .font('Helvetica-Bold')
      .fontSize(11)
      .text(`TOTAL GENERAL: ${formatCurrency(totalGeneral)}`);
    doc.moveDown(1);
