...
export class DashboardService {
  ...
  async getDashboardSummary() {
    ...
    const dineInPercentage = totalChannels > 0 ? Math.round((totalDineIn / totalChannels) * 100) : 0;
    const takeoutPercentage = totalChannels > 0 ? Math.round((totalTakeout / totalChannels) * 100) : 0;
    ...
    if (previousAvgKitchenTime > 0) {
        kitchenTimeTrendPercentage = Math.round(((avgKitchenTimeMinutes - previousAvgKitchenTime) / previousAvgKitchenTime) * 100);
    }
